from model import initialise_graph, update_e, update_degrees, add_vertex, deg_dist_theoretical_pa, save_graph, k_max_pa
import argparse
import pickle
import numpy as np
import time
...
def phase_1_task_4(compute=True, plot=False):
    ...
    if plot:
        ...
        matplotlib.rcParams['mathtext.bf'] = 'Bitstream Vera Sans:bold'
        matplotlib.rcParams['mathtext.fontset'] = 'stix'
        plt.errorbar(N, k_max,  yerr=k_err, marker = '.', ls = ' ', color='b')

        n_space = np.linspace(N[0], N[len(N)-1], 1000)
        plt.plot(n_space, k_max_pa(n_space, m=4), '--', color='k')

        fit_phase, cov_phase = np.polyfit(np.log10(N), np.log10(k_max), 1, cov=True)
        p_phase = np.poly1d(fit_phase)
        plt.plot(n_space, p_phase(n_space), label=r'$Linear \: Fit$')
        ...
        p_collapse = dists[100]/deg_dist_theoretical_pa(x_space[0], m=4)
        k_new = degrees / k

        plt.plot(k_new, p_collapse, 'o')
